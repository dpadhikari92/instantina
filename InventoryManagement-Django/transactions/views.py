    ...
    def get_queryset(self):
        ...
        if search_query:
            queryset = queryset.filter(
                ...
                Q(purchasebillno__stock__name__icontains=search_query)
            ).distinct()
        return queryset
...
def create_bom(request):
    if request.method == 'POST':
        ...
        return redirect('bom-list')  # Replace 'bom-list' with the appropriate URL name for the BOM list view

    # Handle GET request or form validation errors
    raw_materials = Stock.objects.all()  # Retrieve all raw materials
...
def produce_item(request):
    if request.method == 'POST':
        ...
        quantity = float(request.POST.get('quantities'))  # Convert quantity to integer, defaulting to 0 if missing
        ...
        for raw_material in raw_materials:
            ...
            quantity_required = float(bom_raw_material.quantity) * quantity  # Convert bom_raw_material.quantity to integer
            
            if raw_material.quantity < quantity_required:
                messages.error(request, 'Check Inventory level')
        ...
        # Update any other production-related logic here
        
        return redirect('production-list')
    else:
        boms = BOM.objects.all()      
        return render(request, 'production/produce_item.html', {'boms': boms})
...
def produce_list(request):
    produced_items = Production.objects.all()
    
    if request.method == 'POST':
        item_id = request.POST.get('item_id')
        new_status = request.POST.get('new_status')
        
        # Update the status of the item with the given item_id
        item = Production.objects.get(id=item_id)
        item.status = new_status
        item.save()
    
    return render(request, 'production/production_list.html', {'produced_items': produced_items})
